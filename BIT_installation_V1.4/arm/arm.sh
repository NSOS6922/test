...
# find file "BurnInTest_Linux_arm-64.tar.gz" full path and enter into the directory.
cd "$(dirname "$(sudo find / -name 'BurnInTest_Linux_ARM.tar.gz' 2>/dev/null | head -n 1)")"
echo $password | sudo -S sudo  tar -zxvf BurnInTest_Linux_ARM.tar.gz
echo $password | sudo -S sudo chmod 777 * burnintest_arm
sudo cp -r key.dat burnintest_arm/   
sudo cp -r cmdline_config.txt burnintest_arm/
...
# BIT execution
cd "$(dirname "$(sudo find / -name 'bit_cmd_line_aarch64' 2>/dev/null | head -n 1)")"   
#cd $HOME/Desktop/BIT_installation_V1.4/arm/burnintest_arm
echo $password | sudo -S sudo chmod 777 *
echo $password | sudo -S sudo ./bit_cmd_line_aarch64 -k
echo -e "${GREEN}${BOLD}${BLINK}To check license key is avaliable , you'll see Registration successful${RESET}"
echo $password | sudo -S sudo ./bit_cmd_line_aarch64
